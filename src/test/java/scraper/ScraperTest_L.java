...
//		           HtmlPage page =  webClient.getPage("https://www.tarladalal.com/RecipeAtoZ.aspx");
			...
			HtmlAnchor latestPostLink = (HtmlAnchor) Homepage.getByXPath("//div[@id='toplinks']/a[5]").get(0);
			HtmlPage postPage = latestPostLink.click();
			Thread.sleep(3000);

			HtmlPage Lpage = webClient.getPage("https://www.tarladalal.com/RecipeAtoZ.aspx?beginswith=L&pageindex=1");
...
//		            HtmlPage Lpage = anchor.click();
			...
			List<HtmlAnchor> pages = Lpage.getByXPath("//div[@style='text-align:right;padding-bottom:15px;'][1]/a");
			int pagecount = pages.size() + 1;
			System.out.println("No of L Pages = " + pagecount);

			for (int p = 1; p <= pagecount; p++)

			{
				List<HtmlAnchor> nextPageLink = Lpage
						.getByXPath("//div[@style='text-align:right;padding-bottom:15px;'][1]/a[text()='" + p + "']");
